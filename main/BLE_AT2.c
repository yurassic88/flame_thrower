...
void ble_delayed_write(void* params){
    ...
    {
        ESP_LOGI(GATTC_TAG, "Write. gattc_if:%d, conn_id:%d, handle:%d, val_len:%d, write_typ:%d", param->gattc_if, 
                                param->conn_id, 
                                param->handle, 
                                param->value_len,
                                param->write_type);
        ESP_LOGI(GATTC_TAG, "Write auth successful");
        esp_log_buffer_hex(GATTC_TAG, param->value, param->value_len);
    }
...
static void gattc_profile_event_handler(esp_gattc_cb_event_t event, esp_gatt_if_t gattc_if, esp_ble_gattc_cb_param_t *param)
{
    ...
    case ESP_GATTC_SEARCH_CMPL_EVT:
        ...
        if (get_server){
            ...
            if (count > 0) {
                    ...
                    int char_index = get_num_from_uuid(REMOTE_AUTH_RESP_UUID);
                    ...
                    if((char_index!=-1)&& (handle_uuid_map[char_index].properties&ESP_GATT_CHAR_PROP_BIT_WRITE_NR)){
                        ESP_LOGI(GATTC_TAG, "delayed_write");
                        char bArr[]={0x55, 0xAA, 0x00, 0x03, 0xA0, 0x00, 0x00, 0x5C};
                        ...
                        write->value_len=sizeof(bArr);
                        memcpy(write->value, bArr, write->value_len);
                        ...
                        ESP_LOGE(GATTC_TAG, "delayed_write error. char_index:%d", char_index);
                    }
                }
    ...
    case ESP_GATTC_REG_FOR_NOTIFY_EVT:
        ...
        if (p_data->reg_for_notify.status != ESP_GATT_OK){
            ...
            if (!descr_elem_result){
                ...
                else{ESP_LOGI(GATTC_TAG, "esp_ble_gattc_get_descr_by_char_handle returned: %d", count);}
    ...
    case ESP_GATTC_NOTIFY_EVT:
        ...
        if ((p_data->notify.value[0] & 255) == 85 && (p_data->notify.value[1] & 255) == 170 && (p_data->notify.value[4] & 255) == 161) {
                    
            for (int i6 = 5; i6 <= 17; i6 += 2) {
                int i7 = ((i6 - 5) / 2) + 1;
                int probe_id = i7;
                bool ambient = i7==7;
                if((p_data->notify.value[i6]==0xff)&&(p_data->notify.value[i6+1]==0xff)){
                    //ESP_LOGI(GATTC_TAG, "probe id:%d, amb:%d, temp:n/a", probe_id, ambient);
                }
                else{
                    uint16_t temp = (p_data->notify.value[i6] << 8) | p_data->notify.value[i6+1];
                    ESP_LOGI(GATTC_TAG, "probe id:%d, amb:%d, temp:%d", probe_id, ambient, temp);
                }
            }
        }
        if ((p_data->notify.value[0] & 255) == 85 && (p_data->notify.value[1] & 255) == 170 && (p_data->notify.value[4] & 255) == 160) {
            int battery = p_data->notify.value[5] & 255;
            int c_or_f = p_data->notify.value[8] & 255;
    ...
    default:
        ESP_LOGI(GATTC_TAG, "Uknkown event: %d", event);
        break;
    }
}
...
{    
    ...
    esp_err_t local_mtu_ret =  esp_ble_gatt_set_local_mtu(20);
    if (local_mtu_ret){
